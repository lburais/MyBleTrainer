...
module.exports = function(opts) {
  ...
  function open() {
    try {
      if (DEBUG) console.log('[easyUSB.js] - attempting to open device');
      target.open();
    }
    catch (e) {
      if (DEBUG) console.log('[easyUSB.js] - captured error opening device: ', e);
      return device.emit('error', e);
    }

    // reset the device and then open the interface
    if (DEBUG) console.log('[easyUSB.js] - resetting device');
    target.reset(function(err) {
      ...
      if (DEBUG) console.log('[easyUSB.js] - device interfaces: ' + target.interfaces.map(function(iface) {
        return '#' + iface.id;
      }));

      // select the portal interface
      if (DEBUG) console.log('[easyUSB.js] - selecting device interface');
      ...
      if (di.isKernelDriverActive()) {
        if (DEBUG) console.log('[easyUSB.js] - detaching kernel driver');
      ...
      try {
        if (DEBUG) console.log('[easyUSB.js] - claiming interface');
        di.claim();
      }
      catch (e) {
        if (DEBUG) console.log('[easyUSB.js] - error claiming interface: ', e);
        return device.emit('error', e);
      }
      ...
      if (DEBUG) console.log('[easyUSB.js] - device successfully opened');
      device.open = true;
      device.emit('open');
    });
  }
  ...
  **/
  device.close = function(callback) {
    ...
    if (DEBUG) console.log('[easyUSB.js] - releasing interface');
    device.interface.release(function(err) {
      ...
      if (reattach) {
        if (DEBUG) console.log('[easyUSB.js] - reattaching kernel driver');
      ...
      if (DEBUG) console.log('[easyUSB.js] - closing device');
      device.close();
      callback();
    });
  };
  ...
  **/
  device.read = function(size, callback) {
    ...
    if (DEBUG) console.log('[easyUSB.js] - attempting to read ' + size + ' bytes from the input endpoint');
  ...
  **/
  device.write = function(data, callback) {
    ...
    // send the data
    if (DEBUG) console.log('[easyUSB.js] - -> ', data.length, data);
