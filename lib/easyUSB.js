...
module.exports = function(opts) {
  ...
  function open() {
    try {
      message('attempting to open device');
      target.open();
    }
    catch (e) {
      message(`captured error opening device: ${e}`, 'error');
    ...
    target.reset(function(err) {
      ...
      message('device interfaces: ' + target.interfaces.map(function(iface) {
        return '#' + iface.id;
      }));

      // select the portal interface
      message('selecting device interface');
      ...
      if (di.isKernelDriverActive()) {
        message('detaching kernel driver');
      ...
      try {
        message('claiming interface');
        di.claim();
      }
      catch (e) {
        message(`error claiming interface: ${e}`, 'error');
        return device.emit('error', e);
      }
      ...
      message('device successfully opened');
      device.open = true;
      device.emit('open');
    });
  }
  ...
  **/
  device.close = function(callback) {
    ...
    device.interface.release(function(err) {
      ...
      if (reattach) {
        message('reattaching kernel driver');
