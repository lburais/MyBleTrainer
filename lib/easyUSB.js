...
module.exports = function(opts) {
  ...
  function open() {
    try {
      if (DEBUG) logger.info('[easyUSB.js] - attempting to open device');
      target.open();
    }
    catch (e) {
      if (DEBUG) logger.info('[easyUSB.js] - captured error opening device: ', e);
      return device.emit('error', e);
    }

    // reset the device and then open the interface
    if (DEBUG) logger.info('[easyUSB.js] - resetting device');
    target.reset(function(err) {
      ...
      if (DEBUG) logger.info('[easyUSB.js] - device interfaces: ' + target.interfaces.map(function(iface) {
        return '#' + iface.id;
      }));

      // select the portal interface
      if (DEBUG) logger.info('[easyUSB.js] - selecting device interface');
      ...
      if (di.isKernelDriverActive()) {
        if (DEBUG) logger.info('[easyUSB.js] - detaching kernel driver');
      ...
      try {
        if (DEBUG) logger.info('[easyUSB.js] - claiming interface');
        di.claim();
      }
      catch (e) {
        if (DEBUG) logger.error('[easyUSB.js] - error claiming interface: ', e);
        return device.emit('error', e);
      }
      ...
      if (DEBUG) logger.info('[easyUSB.js] - device successfully opened');
      device.open = true;
      device.emit('open');
    });
  }
  ...
  **/
  device.close = function(callback) {
    ...
    if (DEBUG) logger.info('[easyUSB.js] - releasing interface');
    device.interface.release(function(err) {
      ...
      if (reattach) {
        if (DEBUG) logger.info('[easyUSB.js] - reattaching kernel driver');
      ...
      if (DEBUG) logger.info('[easyUSB.js] - closing device');
      device.close();
      callback();
    });
  };
  ...
  **/
  device.read = function(size, callback) {
    ...
    if (DEBUG) logger.info('[easyUSB.js] - attempting to read ' + size + ' bytes from the input endpoint');
  ...
  **/
  device.write = function(data, callback) {
    ...
    // send the data
    if (DEBUG) logger.info('[easyUSB.js] - -> ', data.length, data);
